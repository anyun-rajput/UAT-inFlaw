...
fun startProcess(vararg command: String, redirectErrorStream: Boolean = false): Process =
    ProcessBuilder(*command).directory(UAT.dir).redirectErrorStream(redirectErrorStream).start()

fun startProcess(command: List<String?>, redirectErrorStream: Boolean = false): Process =
    ProcessBuilder(command).directory(UAT.dir).redirectErrorStream(redirectErrorStream).start()

fun runScript(file: File, redirectErrorStream: Boolean = false): Process = if (UAT.win)
    ProcessBuilder("cmd.exe", "/c", file.absolutePath).directory(UAT.dir)
        .redirectErrorStream(redirectErrorStream).start()
else ProcessBuilder("sh", "-c", file.absolutePath).directory(UAT.dir)
    .redirectErrorStream(redirectErrorStream).start()
