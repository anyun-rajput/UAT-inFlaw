                ...
                when (Device.mode) {
                    ...
                    Mode.ADB_ERROR -> {
                        if ("loaded" !in outputTextArea.text)
                            outputTextArea.text =
                                "Note: The device cannot be loaded! 🔃\nTry setting the USB Debugging 🐛 enable \nAnd USB configuration to data transfer 🚌 \nAND launching the application with root/admin privileges 🎫!\n\n"
                    }
                    Mode.FASTBOOT_ERROR -> {
                        if ("loaded" !in outputTextArea.text)
                            outputTextArea.text =
...
// add update data
            ...
            if (!Command.check()) {
                withContext(Dispatchers.Main) {
                    ...
                    Alert(AlertType.WARNING).apply {
                        ...
                        headerText =
                            "Cannot find ADB/Fastboot! Don't Worry\nDownloading the latest version..."
                        ...
                        withContext(Dispatchers.IO) {
                            val file = File(UAT.dir, "platform-tools.zip")
                            when {
                                UAT.win -> Downloader(
                                    "https://dl.google.com/android/repository/platform-tools-latest-windows.zip",
                                    file, label
                                ).start(this)
                                UAT.linux -> Downloader(
                            ...
                            File(UAT.dir, "platform-tools").mkdirs()
                            ZipFile(file).use { zip ->
                                zip.stream().forEach { entry ->
                                    if (entry.isDirectory)
                                        File(UAT.dir, entry.name).mkdirs()
                                    else zip.getInputStream(entry).use { input ->
                                        File(UAT.dir, entry.name).apply {
    ...
    @FXML
    private fun Crossclick(event: MouseEvent){
    ...
    private fun aboutMenuItemPressed(event: ActionEvent) {
        Alert(AlertType.INFORMATION).apply {
            ...
            github.onAction = EventHandler {
                if (UAT.linux)
                    Runtime.getRuntime().exec("xdg-open https://github.com/")
                else Desktop.getDesktop().browse(URI("https://github.com/"))
            }
